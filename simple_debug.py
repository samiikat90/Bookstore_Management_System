...
def test_ajax_simple():
    ...
    try:
        ...
        if '<html' in response.text.lower() or '<!doctype' in response.text.lower():
            print("   ✅ Response is HTML (likely redirect to login)")
            
            # Look for redirect URL
            if 'login' in response.text.lower():
                print("   ✅ Confirmed: Redirecting to login page")
                ...
                if all_orders_response.status_code == 200:
                    ...
                    if 'name="csrf-token"' in all_orders_response.text:
                        ...
                        if csrf_meta_match:
                            ...
                            print("   ❌ Meta tag found but no content")
                        ...
                        print("   ❌ No CSRF meta tag found in page")
                        ...
                        if 'csrf' in all_orders_response.text.lower():
                            print("   ⚠️ Found 'csrf' somewhere in the page")
                        else:
                            print("   ❌ No 'csrf' found anywhere in the page")
                else:
                    print(f"   ❌ Cannot access all_orders: {all_orders_response.status_code}")
            else:
                print("   ❌ Login failed")
        else:
